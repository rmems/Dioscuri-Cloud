...
The storage replacement has landed on AWS S3 (GitHub #47 / #67); the CPU validation host remains
undetermined (see GitHub #52 / #53 training epic). HCP `ibm-dev` / `oracle-dev` workspaces
remain skeleton-only for historical Terraform layout.

| Concern | Provider | Notes |
|---|---|---|
| Canonical artifact store | ~~Oracle~~ → **AWS S3** | Storage only — see `terraform/envs/aws-training` (GitHub #47 / #67). Oracle promo exhausted 2026-06-28; Always Free OCI leftovers are not the SAAQ primary path |
